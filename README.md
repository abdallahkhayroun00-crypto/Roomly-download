...
## Privacy, safe downloads and sharing

The landing page provides three additional areas:

- **Privacy & security** (`#privacy`): a clear, limited overview of what the download website does and what visitors should consider when sharing apartment data. It explicitly does **not** claim end-to-end encryption, a completed audit, or to replace a full legal privacy policy. Before a broader public app launch, review the actual production data processing and publish a properly reviewed privacy policy.
- **Download safety** (`#download-safety`): points visitors to the official GitHub Releases and explains Android APKs, Windows installers / full ZIP extraction, and how to approach system warnings. It does not invent release hashes, versions or installation guarantees.
- **Sharing** (`#share-roomly`, `share.html`): users can copy and share the **public Roomly website URL only**, via native device share, WhatsApp, Telegram or email. An apartment invite code, account or private apartment URL is never generated by this page. The dedicated sharing page supports English, Arabic (RTL), French, Turkish and German and preserves the current language when returning home. The site includes a branded PNG social preview at `assets/roomly-share-card.png`, intended for link previews. External apps and platforms control how those previews ultimately appear.

The static sharing page does not upload a visitor's contacts or messages to Roomly; opening an external sharing app is subject to that service's terms and policies. Clipboard and native sharing require user interaction and suitable browser support.
